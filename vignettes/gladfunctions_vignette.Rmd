...
PAD <-
  readRDS(paste(clean_path, "rds/PAD_Renamed.rds", sep = "/"))
...
A simple one liner to convert all `factor` to `numeric`

```{r convert numeric, eval = FALSE}
PAD_num <- map_df(PAD, ~ if (is.factor(.x)) {
  as.numeric(.x)
} else {
  .x
})
